...
// ❌ Original: const api = process.env.REACT_APP_API_KEY;
// ✅ Next.js App Router: Environment variables must be prefixed with NEXT_PUBLIC_ for client-side use.
// We assume it's correctly set up in the Next.js environment file (e.g., .env.local)
...
const Cart = () => {
  ...
  // localStorage access is safe in a Client Component
  ...
  const [code, setCode] = useState({}); // Initialized to an object for safer access later
    ...
    UserService.PinCodeCheck({
      ...
      .then((res) => {
        if (res?.status === "true") {
          ...
          setPin_Code_Err("");
          setPin_Code_Err("Service available");
          // setPin_Code_Msg("Service not available"); // Original code commented out a message.
          setPin_Code_Valid(true);
          setPin_Code_Loader(false);
        }
      })
  ...
  const [profileData, setProfileData] = useState({}); // Initialized as object
  const [selectedData, setSelectedData] = useState({}); // Initialized as object
        ...
        UserService.PayByPhonepeAPI({
          ...
          .then((res) => {
            if (res?.success === false) {
              ...
              setSpinner(false);
              // ❌ window.location.href usage is fine, but navigation should be handled by Next.js router when possible
  ...
  // 🛑 The logic in this useEffect is complex as it relies on:
  // 1. `location.pathname === "/processing-order"` (checking URL path)
  // 2. `location.search` (transaction ID from query params)
  // 3. `Items` (the cart data)
  // We must update the references to Next.js App Router hooks.
  useEffect(() => {
    // ✅ Next.js App Router: use pathname instead of location.pathname
    if (pathname === "/processing-order") {
      // ✅ Next.js App Router: use searchParams instead of new URLSearchParams(location.search)
  ...
  }, [pathname, searchParams, Items]); // Updated dependencies
  ...
  return (
    <>
      ...
      {/* ✅ Use pathname check instead of location.pathname */}
                              ...
                              <div className="card-body">
                                ...
                                <div className="pt-2">
                                  ...
                                  <button
                                    ...
                                    onClick={() => router.push("/shop")} // ✅ Next.js navigation
                                  >
                                    Back to shop
                                  </button>
                                </div>
